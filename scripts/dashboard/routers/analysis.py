...
router = APIRouter(prefix="/api/backtest", tags=["analysis"])

_bt_cache = {"data": None, "filename": None}
...
def _normalize_trade(trade: Dict[str, Any], source: str, index: int) -> Dict[str, Any]:
    ...
    fee_close = _safe_num(trade.get("fee_close")) * max(stake_amount + _safe_num(trade.get("profit_abs")), 0)
...
def _compare_value(label: str, live: float, bt: float, unit: str = "", digits: int = 2) -> Dict[str, Any]:
...
def _match_score(bt_trade: Dict[str, Any], live_trade: Dict[str, Any]) -> Tuple[float, Dict[str, float]]:
...
def _adverse_entry_slippage_pct(bt_trade: Dict[str, Any], live_trade: Dict[str, Any]) -> float:
...
def _adverse_exit_slippage_pct(bt_trade: Dict[str, Any], live_trade: Dict[str, Any]) -> float:
...
def _diagnose_bucket(match: Dict[str, Any]) -> str:
    ...
    if match["exit_slippage_pct"] >= 0.3:
        return "止损滑点型" if "stop" in match["live_exit_reason"].lower() else "出场滑点型"
    if abs(match["duration_diff_min"]) >= 120 or match["bt_exit_reason"] != match["live_exit_reason"]:
        return "持仓漂移型"
    return "轻微偏移型"


def _summarize_conclusion(contributions: List[Dict[str, Any]], matched_rate: float, bt_only_rate: float, live_only_rate: float) -> str:
    top_reasons = contributions[:3]
    if not top_reasons:
        return "当前只有汇总对比，没有足够的逐笔差异可供分析。"
    reason_text = "，".join(f"{item['label']} {item['share_pct']:.0f}%" for item in top_reasons)
    return (
...
def _build_compare_payload(live_report: Dict[str, Any], bt_report: Dict[str, Any]) -> Dict[str, Any]:
    live_trades = [
    ...
    for bt_trade in bt_trades:
        ...
        score, components, live_trade = sorted(candidates, key=lambda item: item[0], reverse=True)[0]
        used_live_ids.add(live_trade["id"])

        entry_delay_sec = _seconds_between(live_trade["open_dt"], bt_trade["open_dt"]) or 0.0
        exit_delay_sec = _seconds_between(live_trade["close_dt"], bt_trade["close_dt"]) or 0.0
    ...
    live_only = [trade for trade in live_trades if trade["id"] not in used_live_ids]

    metrics = [
        _compare_value("收益率", _safe_num(live_report.get("profit_total")) * 100, _safe_num(bt_report.get("profit_total")) * 100, "%"),
        _compare_value("夏普比率", _safe_num(live_report.get("sharpe")), _safe_num(bt_report.get("sharpe"))),
        _compare_value("索提诺比率", _safe_num(live_report.get("sortino")), _safe_num(bt_report.get("sortino"))),
        _compare_value("卡玛比率", _safe_num(live_report.get("calmar")), _safe_num(bt_report.get("calmar"))),
        _compare_value("获利因子", _safe_num(live_report.get("profit_factor")), _safe_num(bt_report.get("profit_factor"))),
        _compare_value("期望值", _safe_num(live_report.get("expectancy")) * 100, _safe_num(bt_report.get("expectancy")) * 100, "%"),
        _compare_value("最大回撤", _safe_num(live_report.get("max_drawdown_account")) * 100, _safe_num(bt_report.get("max_drawdown_account")) * 100, "%"),
    ]
    ...
    avg_entry_delay = sum(abs(row["entry_delay_sec"]) for row in matched) / matched_count if matched_count else 0.0
    avg_exit_delay = sum(abs(row["exit_delay_sec"]) for row in matched) / matched_count if matched_count else 0.0
    avg_entry_slippage = sum(row["entry_slippage_pct"] for row in matched) / matched_count if matched_count else 0.0
    avg_exit_slippage = sum(row["exit_slippage_pct"] for row in matched) / matched_count if matched_count else 0.0

    contribution_buckets = {
        "入场滑点": sum(max(0.0, row["entry_slippage_pct"]) for row in matched),
        "出场滑点": sum(max(0.0, row["exit_slippage_pct"]) for row in matched),
        "费用与资金费": sum(abs(row["fee_diff"]) + abs(row["funding_fee_diff"]) for row in matched),
        "回测漏单": sum(abs(item["profit_pct"]) for item in bt_only),
        "实盘额外单": sum(abs(item["profit_pct"]) for item in live_only),
        "持仓/时机偏差": sum(max(0.0, abs(row["pnl_diff_pct"]) - max(0.0, row["entry_slippage_pct"]) - max(0.0, row["exit_slippage_pct"])) for row in matched),
    }
    ...
    contributions = [
        ...
        for label, value in sorted(contribution_buckets.items(), key=lambda item: item[1], reverse=True)
        if value > 0
    ]
        ...
        clusters.append(
            {
                ...
                "avg_entry_slippage_pct": round(entry["avg_entry_slippage_pct"] / count, 4),
                "avg_exit_slippage_pct": round(entry["avg_exit_slippage_pct"] / count, 4),
                "avg_pnl_diff_pct": round(entry["avg_pnl_diff_pct"] / count, 4),
            }
        )
    clusters.sort(key=lambda item: item["count"], reverse=True)

    top_anomalies = sorted(matched, key=lambda item: abs(item["pnl_diff_pct"]), reverse=True)[:20]
    ...
    score = max(0.0, min(100.0, matched_rate - bt_only_rate * 0.6 - live_only_rate * 0.5 - max(0.0, avg_entry_slippage) * 12 - max(0.0, avg_exit_slippage) * 12))
    ...
    return {
        ...
        "diagnosis": {
            "score": round(score, 1),
            "conclusion": _summarize_conclusion(contributions, matched_rate, bt_only_rate, live_only_rate),
...
async def upload_backtest(file: UploadFile = File(...)):
    ...
    try:
        if file.filename.endswith(".zip"):
            with zipfile.ZipFile(io.BytesIO(content)) as archive:
                result_files = [
                    ...
                    if item.startswith("backtest-result-") and item.endswith(".json") and not item.endswith("_config.json")
                ]
                if not result_files:
                    raise HTTPException(status_code=400, detail="No backtest-result JSON found in zip")
        ...
        _bt_cache = {"data": bt_data["strategy"][strategy_name], "filename": file.filename}
...
@router.get("/data")
async def get_bt_data():
    return _bt_cache


@router.get("/compare-analysis")
async def compare_analysis(request: Request):
    reporter = request.app.state.reporter
    live_report = reporter.data
    bt_report = _bt_cache.get("data") if _bt_cache else None

    if not live_report or not bt_report:
        return {
            "ready": False,
            "overview": {"metrics": [], "cards": []},
            "diagnosis": {
                "score": None,
                "conclusion": "实盘数据和回测数据未同时准备就绪。",
                "limitations": "需要先生成实盘报告，并上传回测结果文件。",
            },
            "attribution": {"contributions": [], "clusters": []},
            "matching": {"matched": [], "bt_only": [], "live_only": [], "top_anomalies": []},
            "message": "请先生成实盘报告，并上传回测结果文件。",
        }
