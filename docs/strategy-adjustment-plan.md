...
1. **模拟盘运行**（dry_run）：先跑起来，积累 GMGN 数据快照和信号日志
2. **观察信号质量**：用 `view_signals.py` 看 GMGN 覆盖率、信号分布
3. **回测**：用模拟盘积累的 GMGN 快照数据 + 历史 K 线跑回测
4. **对比胜率**：有 GMGN 信号 vs 纯技术面信号的胜率差异
5. **调参**：根据结果调整评分权重和入场门槛
